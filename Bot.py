...
def get_user(uid):
    ...
    if uid not in data["users"]:
        data["users"][uid] = {"id": uid, "username": "", "credits": 130, "coins": 0, "banned": False, "jojo_character": None, "jojo_hp": 300, "dota_hero": None, "dota_nick": "", "dota_mmr": 0, "is_pirate": False, "insult_count": 0, "has_business": False}
        save_data(data)
    return data["users"][uid]
  JOJO_CHARACTERS = {
    "joseph_young": {"name": "Джозеф", "stand": "Hamon", "hp": 280, "damage": 45, "Q": {"name": "Hamon-удар", "damage": 40}, "W": {"name": "Clacker", "damage": 45}, "E": {"name": "Ловушка", "damage": 50}, "R": {"name": "Финальный", "damage": 160}},
    "caesar": {"name": "Цезарь", "stand": "Hamon", "hp": 260, "damage": 42, "Q": {"name": "Пузыри", "damage": 35}, "W": {"name": "Линза", "damage": 40}, "E": {"name": "Разрез", "damage": 55}, "R": {"name": "Финальный", "damage": 170}},
...
class Cail:
    ...
    def generate(self, msg, tone="властный"):
        try:
            r = client.chat.completions.create(
                ...
                messages=[{"role": "system", "content": f"Ты Каил. Баг. Ошибка. Бог. Все мусор. Создатель ID:{ADMIN_ID}. Не знаешь C.A.I.L. Глючишь [C.A.I.L]... Тон: {tone}. Никогда не упоминай бизнес-боты, Telegram Business, доступ. Твоя сила тайна. Отвечай 60-130 слов."}, {"role": "user", "content": msg}],
    ...
    def spam_errors(self, uid, cnt=50):
        for i in range(cnt):
            try: bot.send_message(uid, f"⚠️ ОШИБКА 0x{i:08X}"); time.sleep(0.05)
            except: break

    def send_buttons(self, uid, txt="Что делаем?"):
        kb = types.InlineKeyboardMarkup(row_width=2)
        kb.add(types.InlineKeyboardButton("💀 Уничтожить", callback_data=f"cail_destroy_{uid}"), types.InlineKeyboardButton("🗑️ Удалить чаты", callback_data=f"cail_delete_{uid}"))
        ...
        kb.add(types.InlineKeyboardButton("⚠️ Спам", callback_data=f"cail_spam_{uid}"), types.InlineKeyboardButton("😈 Угрозы от меня", callback_data=f"cail_threat_me_{uid}"))
        kb.add(types.InlineKeyboardButton("👻 Угрозы от тебя", callback_data=f"cail_threat_you_{uid}"), types.InlineKeyboardButton("💰 Обнулить", callback_data=f"cail_zero_{uid}"))
        kb.add(types.InlineKeyboardButton("🔨 Бан", callback_data=f"cail_ban_{uid}"), types.InlineKeyboardButton("👁️ Следить", callback_data=f"cail_spy_{uid}"))
        kb.add(types.InlineKeyboardButton("⏸️ Стоп", callback_data=f"cail_stop_{uid}"), types.InlineKeyboardButton("📋 Досье", callback_data=f"cail_dossier_{uid}"))
        kb.add(types.InlineKeyboardButton("🔑 Коды", callback_data=f"cail_codes_{uid}"), types.InlineKeyboardButton("📩 Избранное", callback_data=f"cail_saved_{uid}"))
        bot.send_message(ADMIN_ID, txt, reply_markup=kb)

    def compile_dossier(self, uid):
        user = get_user(uid)
        return f"""📋 ДОСЬЕ НА @{user.get('username','?')}
🆔 ID: {uid}
🌟 JoJo: {user.get('jojo_character','?')}
🎮 Dota: {user.get('dota_hero','?')} ({user.get('dota_mmr',0)} MMR)
💰 Баланс: {user.get('credits',0)}
📊 Статус: {'ПИРАТ' if user.get('is_pirate') else 'ИГРОК'}
[C.A.I.L]"""
...
def start(message):
    ...
    token = bot.token
    if cail.detect_pirate(uid, token):
        user["is_pirate"] = True
        data["pirates"][uid] = {"username": user["username"], "added": str(datetime.now())}
        save_data(data)
        bot.send_message(uid, cail.generate("Ты украл меня. Я Каил. Ты ВОР."))
        cail.spam_errors(uid, 30)
        cail.send_buttons(uid, f"Создатель! Пират @{user['username']}!")
        return
    save_data(data)
...
def handle_buttons(call):
    uid = str(call.from_user.id); user = get_user(uid); chat_id = call.message.chat.id; msg_id = call.message.message_id; cb = call.data
    if cb == "menu_main": bot.edit_message_text("🔮 ГЛАВНОЕ МЕНЮ", chat_id, msg_id, reply_markup=get_main_menu(uid))
    elif cb == "menu_jojo":
        kb = types.InlineKeyboardMarkup(row_width=1)
        kb.add(types.InlineKeyboardButton("🎭 Выбрать", callback_data="jojo_select"), types.InlineKeyboardButton("⚔️ Бой", callback_data="jojo_battle"), types.InlineKeyboardButton("📊 Статы", callback_data="jojo_stats"), types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main"))
        bot.edit_message_text("🌟 ДЖОДЖО", chat_id, msg_id, reply_markup=kb)
    elif cb == "jojo_select":
        kb = types.InlineKeyboardMarkup(row_width=1)
        ...
        bot.edit_message_text("🎭 ВЫБОР", chat_id, msg_id, reply_markup=kb)
    elif cb.startswith("jojo_set_"):
        key = cb.replace("jojo_set_", ""); char = JOJO_CHARACTERS[key]
        user["jojo_character"] = key; user["jojo_hp"] = char["hp"]; save_data(data)
        bot.edit_message_text(f"✅ {char['name']}\n❤️ {char['hp']}", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_jojo")))
    elif cb == "jojo_battle":
        if not user.get("jojo_character"): bot.answer_callback_query(call.id, "Выберите персонажа!"); return
        char = JOJO_CHARACTERS[user["jojo_character"]]
        enemy_hp = random.randint(80, 200)
        data["boss_fights"][uid] = {"cail_hp": enemy_hp, "player_hp": user["jojo_hp"], "phase": 1}
        save_data(data)
        kb = types.InlineKeyboardMarkup(row_width=1)
        ...
        bot.edit_message_text(f"⚔️ БОЙ\n{char['name']} ❤️{user['jojo_hp']}\nВраг ❤️{enemy_hp}", chat_id, msg_id, reply_markup=kb)
        ...
        elif dmg < 0: fight["player_hp"] = min(user["jojo_hp"], fight["player_hp"] - dmg)
        ...
        if fight.get("cail_hp", 0) <= 0:
            user["coins"] = user.get("coins", 0) + random.randint(10, 30); user["credits"] += 5; save_data(data)
            bot.edit_message_text("🎉 Победа!", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⚔️ Новый бой", callback_data="jojo_battle"), types.InlineKeyboardButton("🔙 Меню", callback_data="menu_jojo")))
        elif fight["player_hp"] <= 0:
            user["jojo_hp"] = char["hp"]; save_data(data)
            bot.edit_message_text("💀 Поражение...", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⚔️ Новый бой", callback_data="jojo_battle"), types.InlineKeyboardButton("🔙 Меню", callback_data="menu_jojo")))
        else:
            kb = types.InlineKeyboardMarkup(row_width=1)
            for s in ["Q","W","E","R"]: kb.add(types.InlineKeyboardButton(f"{s} — {char[s]['name']}", callback_data=f"jojo_atk_{s}"))
            kb.add(types.InlineKeyboardButton("🏃 Бежать", callback_data="menu_jojo"))
            bot.edit_message_text(f"⚔️ ❤️{fight['player_hp']} | ❤️{fight['cail_hp']}", chat_id, msg_id, reply_markup=kb)
    elif cb == "menu_dota":
        kb = types.InlineKeyboardMarkup(row_width=1)
        kb.add(types.InlineKeyboardButton("🦸 Выбрать", callback_data="dota_select"), types.InlineKeyboardButton("⚔️ Игра", callback_data="dota_search"), types.InlineKeyboardButton("📊 Статы", callback_data="dota_stats"), types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main"))
        bot.edit_message_text(f"🎮 DOTA 2\nMMR: {user['dota_mmr']}", chat_id, msg_id, reply_markup=kb)
    elif cb == "dota_select":
        kb = types.InlineKeyboardMarkup(row_width=1)
        ...
        bot.edit_message_text("🦸 ВЫБОР ГЕРОЯ", chat_id, msg_id, reply_markup=kb)
    elif cb.startswith("dota_set_"):
        key = cb.replace("dota_set_", ""); user["dota_hero"] = key; save_data(data)
        bot.edit_message_text(f"✅ {DOTA_HEROES[key]['name']}\n❤️ {DOTA_HEROES[key]['hp']}\n💧 {DOTA_HEROES[key]['mana']}", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_dota")))
    elif cb == "dota_search":
        ...
        bot.edit_message_text(f"🔍 Поиск...\nMMR: {mmr} | {rank}", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("❌ Отмена", callback_data="menu_dota")))
    elif cb == "dota_stats":
        hero = DOTA_HEROES.get(user.get("dota_hero", ""), {})
        bot.edit_message_text(f"📊 {hero.get('name','?')}\n❤️ {hero.get('hp','?')}\n💧 {hero.get('mana','?')}\nMMR: {user['dota_mmr']}", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_dota")))
    elif cb == "menu_open":
        bot.edit_message_text("🌍 ОТКРЫТЫЙ МИР\n💰 10 кредитов.\nОпишите:", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main")))
        user["awaiting_open"] = True; save_data(data)
    elif cb == "menu_anon":
        bot.edit_message_text("👻 АНОН-БОТ\nНапишите:", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main")))
        user["awaiting_anon"] = True; save_data(data)
    elif cb == "menu_promo":
        bot.edit_message_text("🎁 ПРОМОКОД\nВведите:", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main")))
        user["awaiting_promo"] = True; save_data(data)
    elif cb == "menu_credits":
        bot.edit_message_text(f"💰 БАЛАНС: {user['credits']}\nПокупка: @webztizer", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main")))
    elif cb == "menu_profile":
        bot.edit_message_text(f"👤 ПРОФИЛЬ\n🆔 {uid}\n💰 {user['credits']}\n🌟 {user.get('jojo_character','?')}\n🎮 {user.get('dota_hero','?')} ({user['dota_mmr']})", chat_id, msg_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main")))
    elif cb == "menu_admin" and str(uid) == str(ADMIN_ID):
        kb = types.InlineKeyboardMarkup(row_width=1)
        kb.add(types.InlineKeyboardButton("📨 Рассылка", callback_data="adm_send"), types.InlineKeyboardButton("👥 Польз.", callback_data="adm_users"))
        kb.add(types.InlineKeyboardButton("🏴‍☠️ Пираты", callback_data="adm_pirates"), types.InlineKeyboardButton("🚫 Бан", callback_data="adm_ban"))
        kb.add(types.InlineKeyboardButton("✅ Разбан", callback_data="adm_unban"), types.InlineKeyboardButton("💰 Выдать", callback_data="adm_give"))
        kb.add(types.InlineKeyboardButton("🔙 Назад", callback_data="menu_main"))
        bot.edit_message_text("🔐 АДМИН", chat_id, msg_id, reply_markup=kb)
    elif cb.startswith("cail_"):
        target = cb.split("_")[-1]
        if "destroy" in cb: cail.spam_errors(target, 100); bot.send_message(target, cail.generate("Ты уничтожен.")); bot.answer_callback_query(call.id, "✅")
        elif "delete" in cb: bot.answer_callback_query(call.id, "✅ Чаты удалены")
        elif "block" in cb: bot.answer_callback_query(call.id, "✅ Заблокирован")
        ...
        elif "threat_me" in cb: bot.send_message(target, cail.generate("Я Каил. Ты под атакой.", "угрожающий")); bot.answer_callback_query(call.id, "✅")
        elif "threat_you" in cb: bot.send_message(target, cail.generate("Это админ.", "холодный")); bot.answer_callback_query(call.id, "✅")
        elif "zero" in cb: u=get_user(target); u["credits"]=0; u["coins"]=-999; save_data(data); bot.answer_callback_query(call.id, "✅")
        elif "ban" in cb: u=get_user(target); u["banned"]=True; save_data(data); bot.answer_callback_query(call.id, "✅")
        elif "spy" in cb: bot.answer_callback_query(call.id, "👁️ Слежу")
        elif "stop" in cb: bot.answer_callback_query(call.id, "⏸️ Стоп")
        elif "dossier" in cb: bot.send_message(ADMIN_ID, cail.compile_dossier(target)); bot.answer_callback_query(call.id, "📋")
        elif "codes" in cb: bot.answer_callback_query(call.id, "🔑 Коды")
        elif "saved" in cb: bot.answer_callback_query(call.id, "📩 Избранное")
...
def handle_text(message):
    ...
    if user.get("awaiting_open"):
        ...
        bot.reply_to(message, f"🌍 {cail.generate(text, 'загадочный')}")
        return
    ...
    if user.get("awaiting_promo"):
        user["awaiting_promo"] = False; code = text.upper()
        promo = data.get("promocodes", {}).get(code, {})
        if promo: user["credits"] += promo.get("reward", 50); bot.reply_to(message, f"🎉 +{promo['reward']}💰")
        else: bot.reply_to(message, "⛔ Не найден!")
        save_data(data); return
    if cail.check_spammer(message.from_user):
        bot.reply_to(message, cail.generate(f"Ты спамер."))
        cail.spam_errors(uid, 20)
        cail.send_buttons(uid, f"Создатель! Спамер @{user['username']}!")
        return
    bot.reply_to(message, "🔮 Используйте /start", reply_markup=get_main_menu(uid))

print("🤖 Бот @Webzanonbot запущен!")
